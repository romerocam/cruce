...
export default async function handler(req, res) {
  ...
  if (!session)
    res.status(401).json({ title: `Login`, message: "Not Authenticated!" }); // return implicito
  ...
  switch (method) {
    case "GET": // busca el usuario del userId pasado por params:
      try {
        ...
        ); // ObjectId convierte el string a ObjetId de mongo
        res.status(200).json({
          success: true,
          data: foundUser,
          title: `Find User`,
    ...
    case "DELETE": // si no existe el usuario responde con 409 y un mensaje, sino lo borra:
      try {
        ...
        if (!existingUser) {
          res
            .status(409)
            .json({ success: false, data: `User ${email} does not exist` });
        } else {
          const deletedQuantity = await User.deleteOne({
            _id: ObjectId(userId),
          });
